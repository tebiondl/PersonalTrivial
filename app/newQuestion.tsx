...
export default function NewQuestion() {
  ...
  const saveQuestion = async () => {
    try {
      ...
      const newId = questionsArr.length > 0 ? Math.max(...questionsArr.map((q: { id: number }) => q.id)) + 1 : 1;
      const newQuestionObj = { id: newId, question, answer };
      questionsArr.push(newQuestionObj);
      await FileSystem.writeAsStringAsync(fileUri, JSON.stringify(questionsArr, null, 2));
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDarkMode ? '#333' : '#fff' }]}>
      ...
      <Text style={[styles.title, { color: isDarkMode ? '#fff' : '#000' }]}>Create a New Question</Text>
      <TextInput
...
const styles = StyleSheet.create({
  ...
  input: { height: 40, borderColor: 'gray', borderWidth: 1, width: '80%', marginBottom: 20, paddingHorizontal: 10, borderRadius: 8 },
